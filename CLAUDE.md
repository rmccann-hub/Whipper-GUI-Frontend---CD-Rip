...
- **Read-offset lookup by drive model — auto-detect refactor (2026-06-05):** Real-hardware test: whipper's `offset find` failed even with a *recognizable* CD (it's "primitive" + depends on in-container AccurateRip working). Root cause = the architecture leaned entirely on that probe and ignored the drive vendor/model we already parse. Fix ([docs/offset-investigation-2026-06.md](docs/offset-investigation-2026-06.md)): new `adapters/accuraterip_offsets.py` (`OffsetDatabase`) resolves the offset by drive model the way EAC/dBpoweramp do — no disc. `DrivePicker.current_drive()` exposes the descriptor; `DriveSetupDialog` pre-fills the known offset (`known_offset`/`drive_label`) for one-click Save; `MainWindow._on_drive_setup` does the lookup; whipper `offset find` demoted to optional verify. Curated in-code table (kept in code to dodge AppImage package-data pitfalls), led by Pioneer family (BDR-209D=+667, user-confirmed), extensible via `~/.config/whipper-gui/drive_offsets.csv`. **Safety:** only ever *suggests* (wrong offset corrupts rips → human confirms). **Gated future work:** import the full AccurateRip `DriveOffsets.bin` (binary format needs hardware validation) + verify other drives before adding to the curated table. Tests 534→556 (TASKS #8 now `[~]`). NOTE: I could NOT have the user validate live (they're away) — the +667 path is unit-tested + user-pre-confirmed, but a real from-scratch wizard run on hardware is still the final proof.

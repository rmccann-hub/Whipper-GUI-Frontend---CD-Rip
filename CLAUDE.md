...
- **Critical Rule #3 — one approved exception (2026-05-31): force-stopping a cancelled rip.** A rip's `cdparanoia` runs inside the `ripping` container; podman doesn't forward the host SIGKILL, so the drive can spin for minutes after Cancel. The user approved a narrow exception: `drive_control.force_stop_drive()` tries a host `eject` first, then `distrobox enter ripping -- pkill -f 'cdparanoia|cd-paranoia|whipper'` — the GUI calling into the container, which Rule #3 otherwise forbids. **Scope is strictly force-stop on cancel** — ripping still goes through `~/.local/bin/whipper`. MainWindow auto-escalates `_FORCE_STOP_COUNTDOWN_MS` (8s) after Cancel, plus a manual "Force stop" button. Do NOT widen this exception without asking.

...
5. **CTDB verify (read-only)** — Phase 1 of KDD-14; foundation for repair. **← next up, but needs hardware + a license check first** (raised 2026-06-02; see [docs/upstream-modification-investigation.md](docs/upstream-modification-investigation.md)): the verify CRC can only be validated against a real CD that's in CTDB, and the Python reference is GPL-2.0 vs our GPL-3.0-only. Pure-Python client (not a `ctdb-cli` wrap).
...
- **[HIGH] CTDB verify (read-only).** KDD-14 Phase 1. Not a whipper change — a new pure-Python `CTDBClient` adapter (modeled on the LGPL server reference / `bmwalters/python-cuetoolsdb`), **not** a wrap of `ctdb-cli`. Concrete spec in [docs/upstream-modification-investigation.md](docs/upstream-modification-investigation.md). **Two gates before coding (raised 2026-06-02):** (1) the verify CRC runs over decoded FLAC audio and can only be validated against a real CD that's in CTDB — a hardware (T32-style) test; (2) a GPL-2.0→GPL-3.0-only license check on the Python reference (reimplement clean-room from the LGPL server if it's GPL-2.0-only).

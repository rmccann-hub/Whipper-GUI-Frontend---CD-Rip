...
7. **[ ] ⭐ HIGH PRIORITY — Zero-CLI distribution (PLANNING.md [KDD-17](PLANNING.md), user-approved 2026-06-04).** Goal: a non-technical user touches no terminal — download one file, double-click, done. **This is the top-ranked *remaining codeable* item for end-user impact** — do it ahead of CTDB repair (6); it's independent of the hardware-gated CTDB verify (5), so it can proceed now. Three independently-shippable pieces, in order: (a) **self-integrate on first run** — the AppImage offers to install its own `.desktop` + icon (supersedes user-facing `install-appimage.sh`); (b) **self-update** — embed AppImage update-information (zsync) + verify against the published `.sha256` (the standard "skip if current, else fetch delta" mechanism — do *not* hand-roll a download stub); (c) **GUI first-run host wizard** — the larger lift: move `setup-host.sh`'s work (create the `ripping` container, install whipper/flac/metaflac in it, `distrobox-export`) into a buttons-and-progress wizard owned by the dependency self-management subsystem (Critical Rule #6), driving rootless podman/distrobox directly; one polkit-elevated step only where the container *runtime* is missing (rootless + no prompt on Bazzite/Silverblue, where it's preinstalled). Rejected: Flatpak (sandbox can't reach host whipper — Critical Rule #3) and a bespoke download stub (AppImage update-info already does it). See full rationale in KDD-17. Detailed entry under P1.1 below.
8. **[~] Auto drive-offset lookup (backend-independent; high-value).** Surfaced by real-hardware testing: whipper's `offset find` is "primitive" and failed on a Pioneer BDR-209D even with a recognizable CD. **Phase 1 SHIPPED 2026-06-05** ([docs/offset-investigation-2026-06.md](docs/offset-investigation-2026-06.md)): `adapters/accuraterip_offsets.py` looks the offset up by drive vendor+model and the wizard pre-fills it for one-click save (no disc, no whipper probe); curated in-code table led by the Pioneer family (BDR-209D=+667 confirmed), extensible via `~/.config/whipper-gui/drive_offsets.csv`. **Remaining (gated):** import the full AccurateRip `DriveOffsets.bin` list (binary-format validation needs real hardware + known-good drives), and verify additional drives' offsets before adding to the curated table.

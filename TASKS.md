...
8. **[ ] Auto drive-offset lookup (backend-independent; high-value, small).** Surfaced by real-hardware testing 2026-06-04: whipper's `offset find` failed on a Pioneer BDR-209D (its detection is "primitive"), forcing the manual `+667`. Auto-look-up the read offset from the **AccurateRip drive-offset list keyed by the drive's model** (which we already display), and pre-fill/save it so the user never types a number. Independent of the whipper-vs-cyanrip choice; would have prevented today's friction. Pairs with the manual fallback already in the wizard.

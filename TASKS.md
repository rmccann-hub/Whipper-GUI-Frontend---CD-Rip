...
- **[~] Tag `v0.0.1` and publish the AppImage as a release asset.** Promotes Method A to "the recommended path" and removes the "AppImage not yet published" caveat. Pre-req met: AppImage build verified end-to-end on hardware (T32). **Release prep done 2026-05-31** (branch `claude/nifty-ritchie-U3fDm`): README rewritten for a published-AppImage world (tester quickstart, Method A download path, Uninstalling section, dropped pre-alpha/private-repo caveats) and `CHANGELOG.md` added (doubles as the release body). Real app icon added 2026-05-31 (`build/make_icon.py` → committed `build/python-appimage/whipper-gui.png`, a 512×512 CD-disc icon; replaces the 16×16 grey placeholder, verified bundled into a fresh AppImage build). **Remaining (manual, needs GitHub UI):** merge the branch to `main`, rebuild the AppImage from `main` (`bash build/build_appimage.sh`), create the `v0.0.1` GitHub Release, and upload `whipper-gui-x86_64.AppImage` as the asset.

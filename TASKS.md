...
- **[x] Show numbered track rows for an unknown disc.** Done 2026-05-29 (T32). Previously the track table stayed empty when MusicBrainz had no match, so the user couldn't see what was on the disc before an unknown-album rip. whipper reports the track count even for an unidentified disc, so the adapter now salvages it from `cd info`'s partial output (`DiscInfo.num_tracks`) and the main window renders that many numbered blank rows via `TrackTable.set_blank_tracks()`. The no-match handler is shared by the empty-disc-ID path and the 0-result-lookup path. **Follow-up is P2 (below):** edits to those rows don't yet feed the unknown rip — whipper writes placeholder `Track NN` tags and the post-rip step applies them.

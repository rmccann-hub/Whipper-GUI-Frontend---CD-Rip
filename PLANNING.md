...
- **Cover art: embed where the toolchain allows, folder-image always.** FLAC (metaflac PICTURE) and MP3 (ffmpeg → ID3 APIC) embed the front cover; the album-folder `cover.<ext>` (written by `cover_art.py`) is the **universal** image for every format. WavPack is the one gap: **ffmpeg's WavPack muxer rejects a second stream**, so it can't embed art *in* the `.wv` — embedding needs the standalone `wavpack` tool (APEv2 binary tag), deferred as a documented enhancement. WAV embeds nothing (RIFF). This is why **WavPack, not WAV, is the recommended lossless-with-metadata format** — WAV gets a UI warning.
